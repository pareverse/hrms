...
const Profile = () => {
	...
	const { data: user, isFetched: isUserFetched } = useQuery(['user', id], () => api.get('/users', session.user.id))
							...
							<Flex direction="column" gap={6}>
								...
								<Flex align="center" direction={{ base: 'column', md: 'row', lg: 'column', xl: 'row' }} gap={6}>
									<FormControl>
										...
										<Select defaultValue={user.department} size="lg" disabled={session.user.role !== 'Admin'} {...register('department', { required: true })}>
									...
									<FormControl>
										...
										<Select defaultValue={user.designation} size="lg" disabled={session.user.role !== 'Admin'} {...register('designation', { required: true })}>
